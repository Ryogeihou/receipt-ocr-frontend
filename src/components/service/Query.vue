<template>
  <div>
    ...
    <el-card>
      <el-button class="add" size="small" type="primary" native-type="button"
        @click="add()"
        >new record</el-button>
      <el-table :data="orderList" border stripe>
        <el-table-column label="items" type="expand" prop="itemList" width="58px">
      ...
      <el-dialog class="signUpBox" :title="title" :visible.sync="editDialogVisible" width="50%">
        <el-form :model="editForm" ref="editFormRef" label-width="100px" :rules="editFromRules">
          ...
          <el-form-item label="Items">
            <div v-for="(row, i) in editForm.items" :key="i">
              <el-input style="width: 230px" v-model="editForm['items'][i]"></el-input>
              <el-input @input="getTotal()" style="width: 80px" v-model="editForm['price'][i]"
              ></el-input>
              <el-button class="delBtn" type="text" size="mini" @click="deleteItem(i)">
                Del</el-button></div>
          </el-form-item>
          <el-form-item>
          ...
          <el-form-item label="Total">
            <el-input style="width: 310px" v-model="editForm.totalAmount"></el-input>
            <el-button class="delBtn" type="text" size="mini" @click="getTotal()">
              getTotal</el-button>
          </el-form-item>
        </el-form>
...
//created ()はライフサイクルフックと呼ばれ、
// vue.jsの初期化の中の決められたタイミングで実行される関数です。
  created () {
    this.query()
  },
  ...
  methods: {
    async query () {
      ...
      orderList.map(v => this.getJson(v)) // 商品名
